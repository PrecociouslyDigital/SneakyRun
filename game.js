...
var Notes = [new buzz.sound("c.wav"),
new buzz.sound("e.wav"),
new buzz.sound("g.wav"),
new buzz.sound("c.wav")];
var beat = new buzz.sound("CH.mp3");
...
game.bop = {
  ...
  with_key: function(key){
    if (!!(game.activeMole + 1) === true && key === game.holes[game.activeMole].label){
      Notes[game.activeMole].stop();
      Notes[game.activeMole].play();
...
game.hole = {
  ...
  draw: function(){
    ...
    this.drawLabel();
    if (this.active === true){
      this.drawMole(this.holeLocation[0], this.holeLocation[1]);
    };
  },
  drawHole: function(){
